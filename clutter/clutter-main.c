...
static gboolean clutter_show_frame_time = FALSE;
...
/**
 * clutter_get_show_frame_time:
 *
 * Returns whether Clutter should print out the time a frame took to render
 * on the console. You can enable this setting either using the
 * <literal>CLUTTER_SHOW_FRAME_TIME</literal> environment variable or passing
 * the <literal>--clutter-show-frame-time</literal> command line argument. *
 *
 * Return value: %TRUE if Clutter should show the FPS.
 *
 * Since: 0.4
 */
gboolean
clutter_get_show_frame_time (void)
{
  return clutter_show_frame_time;
}
...
clutter_redraw (ClutterStage *stage)
{
  ...
  */
  ...
  if (G_UNLIKELY (clutter_get_show_frame_time ()))
    {
        g_print("*** Frame %d ms ***\n", (int)(g_timer_elapsed (timer_frame, NULL) * 1000.0));
        g_timer_destroy(timer_frame);
    }
...
static GOptionEntry clutter_args[] = {
  { "clutter-show-fps", 0, 0, G_OPTION_ARG_NONE, &clutter_show_fps,
   ...
   { "clutter-show-frame-time", 0, 0, G_OPTION_ARG_NONE, &clutter_show_frame_time,
